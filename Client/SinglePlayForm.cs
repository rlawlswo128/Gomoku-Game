...
namespace Client
{
    public partial class SinglePlayForm : Form
    {
        ...
        private bool judge() // 승리 판정 함수
        {
            ...
            for(int i = 0; i < edgeCount - 4; i++) // Y = X, 대각선
            {
            ...
            for(int i = 4; i > edgeCount; i--) // Y = -X, 대각선
            {
                for(int j = 0; j < edgeCount - 4; i++)
                {
                    if (board[i, j] == nowPlayer && board[i - 1, j + 1] == nowPlayer && board[i - 2, j + 2] == nowPlayer && board[i - 3, j + 3] == nowPlayer && board[i - 4, j + 4] == nowPlayer)
                        return true;
                }
        ...
        private void boardPictureBox_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            Graphics g = this.boardPictureBox.CreateGraphics();
            int x = e.X / rectSize;
            int y = e.Y / rectSize;
        ...
        private void boardPictureBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            gp.DrawLine(p, rectSize / 2, rectSize / 2, rectSize * edgeCount - rectSize / 2, rectSize / 2); // 우측
